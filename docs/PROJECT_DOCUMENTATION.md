...
Если у чата ещё нет HTTP diagnostics, порядок отображения строится по `id ASC`, то есть по фактическому порядку вставки событий в базу. Если diagnostics уже есть, экран чата восстанавливает таймлайн из `http_events`, потому что именно там хранится реальный порядок прихода stream-событий.
...
Для сохранённых stream-чатов UI-таймлайн строится из `http_events` по `id ASC`, а не из уже сохранённых agent/artifact rows в `messages`. Это нужно, чтобы старые чаты, записанные прежней логикой в неправильном порядке, отображались по реальному порядку прихода events из diagnostics.

Для обычного JSON response extractor проходит по ключам объекта в порядке JSON. Например:
...
```text
status -> history/messages -> artifacts
```
...
Приложение не должно переставлять terminal status в конец или искусственно сортировать response.
