...
- Для каждой операции можно указать отдельную ручку. Если поле пустое, используется дефолт: JSON-RPC методы уходят на основной endpoint, Agent Card - на `/.well-known/agent-card.json` от host.
