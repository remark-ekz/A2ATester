...
Порядок отображения строится по `id ASC`, то есть по фактическому порядку вставки событий в базу. Это важно для stream-сценариев, где несколько событий могут попасть в одну секунду.
